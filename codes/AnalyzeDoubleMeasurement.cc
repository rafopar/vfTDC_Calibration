...
void GetOrganizedHits(vector<tdcHit>& v_allHits, vector< vector<tdcHit*> > v_organizedHits) {
    ...
    for (it1 = v_allHitsptr.begin(); it1 != v_allHitsptr.end(); it1++) {
        vector<tdcHit*> v_cutHits;
        v_cutHits.push_back(*it1);
        ...
        v_organizedHits.push_back(v_cutHits);
    }

    if (v_allHits.size() > 4) {
        for (auto cur_v : v_organizedHits) {
            for (auto curHit : cur_v) {
                cout << curHit->interval << "   ";
            }
            cout << endl;
        }
        cout << "Size of v_organizedHits " << v_organizedHits.size() << endl;
    }
}